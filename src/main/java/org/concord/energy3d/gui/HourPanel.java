...
class HourPanel extends JPanel {
    ...
    HourPanel(Foundation foundation, ThermostatView[] thermostatButtons) {
    ...
    private void processMouseReleased(MouseEvent e) {
        ...
        if (selectedHour >= 0) {
            ...
            for (ThermostatView thermostatButton : thermostatButtons) {
                Object[] keys = thermostatButton.hourlyTemperatures.keySet().toArray();
                Object selectedKey = keys[selectedHour];
                int newTemperature = thermostatButton.hourlyTemperatures.get(selectedKey);
                if (increaseTemperature)
                    newTemperature++;
                if (decreaseTemperature)
                    newTemperature--;
                thermostatButton.hourlyTemperatures.put((Float) selectedKey, newTemperature);
                foundation.getThermostat().setTemperature(thermostatButton.monthOfYear, thermostatButton.dayOfWeek, selectedHour, newTemperature);
                thermostatButton.repaint();
            }
            if (selectedHour == 24) {
                for (ThermostatView thermostatButton : thermostatButtons) {
                    Object[] keys = thermostatButton.hourlyTemperatures.keySet().toArray();
                    for (int h = 0; h < 24; h++) {
                        int t2 = thermostatButton.hourlyTemperatures.get(keys[h]);
                        if (increaseTemperature)
                            t2++;
                        if (decreaseTemperature)
                            t2--;
                        thermostatButton.hourlyTemperatures.put((Float) keys[h], t2);
                        foundation.getThermostat().setTemperature(thermostatButton.monthOfYear, thermostatButton.dayOfWeek, h, t2);
                    }
                    thermostatButton.repaint();
                }
            }
