...
public class PopupMenuFactory {
						...
						if (choice == options[1]) {
							...
							if (ok) {
								...
								if (w < 0.1 || w > wmax) {
									...
									if (rb1.isSelected()) {
										final SetPartShapeCommand c = new SetPartShapeCommand(window);
		...
		if (popupMenuForFoundation == null) {
			...
			popupMenuForFoundation = createPopupMenu(false, true, new Runnable() {
				...
				public void run() {
					...
					miPaste.setEnabled(copyBuffer instanceof SolarPanel || copyBuffer instanceof Mirror || copyBuffer instanceof Rack || copyBuffer instanceof ParabolicTrough || copyNode != null);
				}
			});
						...
						if (choice == options[1]) {
							...
							if (ok) {
								if (w < 1 || w > 500) {
									...
									if (rb1.isSelected()) {
										final SetPartShapeCommand c = new SetPartShapeCommand(rack);
										rack.setRackWidth(w);
						...
						if (choice == options[1]) {
							...
							if (ok) {
								if (w < 1 || w > 50) {
									...
									if (rb1.isSelected()) {
										final SetPartShapeCommand c = new SetPartShapeCommand(m);
										m.setMirrorWidth(w);
										m.setMirrorHeight(h);
										m.draw();
										SceneManager.getInstance().getUndoManager().addEdit(c);
	...
	private static JPopupMenu getPopupMenuForParabolicTrough() {
		...
		if (popupMenuForParabolicTrough == null) {
			...
			final JMenuItem miShape = new JMenuItem("Shape...");
			miShape.addActionListener(new ActionListener() {
