...
class PropertiesDialogForFresnelReflector extends PropertiesDialogFactory {
	...
	static JDialog getDialog(final FresnelReflector reflector) {
		...
		final JTextField totalLengthField = new JTextField(PopupMenuFactory.threeDecimalsFormat.format(reflector.getLength()) + " m (" + reflector.getNumberOfModules() + "\u00D7" + PopupMenuFactory.threeDecimalsFormat.format(reflector.getModuleLength()) + " m)");
