...
class GeneTemporalGraph extends AbstractGraph {
	...
	public GeneTemporalGraph(final Individual[] individuals, final int gene, final double minGene, final double maxGene) {
		...
		for (final Individual i : individuals) { // they may converge earlier so we have to count here
			if (i != null) {
				length++;
			}
		}
	...
	public void update(final Graphics g) {
		...
		for (int i = 0; i <= ny; i++) {
			...
			if (i == 0 || i == ny) {
				tickmarkLabel = EnergyPanel.TWO_DECIMALS.format(minGene + i * (maxGene - minGene) / ny);
		...
		for (int i = 0; i < individuals.length; i++) {
			...
			xTick = x0 + i * dx;
			if (Util.isEqual(minGene, maxGene, 0.000001)) {
				yTick = xAxisY - graphWindowHeight * 0.5;
			} else {
				final double v = minGene + individuals[i].getGene(gene) * (maxGene - minGene);
				yTick = xAxisY - (v - minGene) / (maxGene - minGene) * graphWindowHeight;
			}
			if (path == null) {
