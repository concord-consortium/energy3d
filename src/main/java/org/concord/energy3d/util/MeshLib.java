...
public class MeshLib {
    ...
    public static void applyHoles(final Node root, final List<Window> windows, final int textureType) {
        final Map<Window, List<ReadOnlyVector3>> holes = new HashMap<>();
        for (final Window window : windows) {
            final ArrayList<ReadOnlyVector3> hole = new ArrayList<>();
        ...
        for (int roofIndex = 0; roofIndex < root.getChildren().size(); roofIndex++) {
            ...
            if (roofPart.getSceneHints().getCullHint() != CullHint.Always) {
                ...
                for (final Window window : windows) {
                    ...
                    final List<PolygonPoint> holePolygon = new ArrayList<>();
    ...
    public static ArrayList<ReadOnlyVector3> computeOutline(final FloatBuffer buf) {
        final Map<LineSegment3, Boolean> visitMap = new HashMap<>();
