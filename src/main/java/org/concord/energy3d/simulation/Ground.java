...
public class Ground implements Serializable {
    ...
    double getAdjustedAlbedo(final int month) {
        if (snowReflectionFactors == null) {
            snowReflectionFactors = new double[12];
            Arrays.fill(snowReflectionFactors, 0);
            return albedo;
    ...
    int getDailyLagInMinutes() {
        return dailyLagInMinutes;
    }

    private double getTemperatureOnDay(final int day) {
    ...
    private double getTemperatureOnDay(final int day, final double depth) {
        ...
        ave /= 2 * n;
        double amp = 0.25 * (hiMax - hiMin + loMax - loMin);
    ...
    double getTemperatureMinuteOfDay(final int day, final int minute, final double airTemperatrureFluctuationAmplitudeOfDay) {
        return getTemperatureOnDay(day) - Math.exp(-foundationDepth * Math.sqrt(OMEGA_DAY / (2.0 * thermalDiffusivity))) * airTemperatrureFluctuationAmplitudeOfDay
                * Math.cos(OMEGA_DAY * (minute - dailyLagInMinutes));
    }

    double getTemperatureMinuteOfDay(final int day, final int minute, final double airTemperatrureFluctuationAmplitudeOfDay, final double depth) {
        return getTemperatureOnDay(day, depth) - Math.exp(-depth * Math.sqrt(OMEGA_DAY / (2.0 * thermalDiffusivity))) * airTemperatrureFluctuationAmplitudeOfDay
                * Math.cos(OMEGA_DAY * (minute - dailyLagInMinutes));
    }
