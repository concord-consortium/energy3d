...
public abstract class Graph extends JPanel {
			...
			public void popupMenuWillBecomeVisible(final PopupMenuEvent e) {
				...
				if (!popup) {
					...
					mi.addActionListener(new ActionListener() {
						@Override
						public void actionPerformed(final ActionEvent e) {
							final int i = JOptionPane.showConfirmDialog(JOptionPane.getFrameForComponent(Graph.this), "Are you sure that you want to clear all the previous results\nrelated to the selected object?", "Confirmation", JOptionPane.YES_NO_OPTION);
	...
	public void hideData(final String name, final boolean hidden) {
		hideData.put(name, hidden);
	}

	public boolean isDataHidden(final String name) {
		final Boolean b = hideData.get(name);
		return b != null ? b : false;
	}

	public void hideRun(final int id, final boolean hidden) {
		hideRuns.put(id, hidden);
	}

	public boolean isRunHidden(final int id) {
	...
	void drawRecords(final Graphics2D g2) {
	...
	void drawHorizontalLine(final Graphics2D g2, final int yValue, final String yLabel) {
		g2.setStroke(thin);
		g2.setColor(Color.LIGHT_GRAY);
