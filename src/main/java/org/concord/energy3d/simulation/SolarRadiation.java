...
public class SolarRadiation {
    ...
    // public final static double[] ASHRAE_C = new double[] { 0.058, 0.060, 0.071, 0.097, 0.121, 0.134, 0.136, 0.122, 0.092, 0.073, 0.063, 0.057 };
    ...
    private class MeshDataStore { // renamed this to avoid name conflict with MeshData
        ...
        public int rows;
        public int cols;
        double[][] dailySolarIntensity; // store the solar energy intensity distribution on this mesh
        double[] solarPotential; // store the total solar energy that falls onto this mesh -- this applies to only roof meshes and imported meshes (other parts that have a single mesh uses its own array)
        double[] heatLoss;
    }
    ...
    private void computeToday() {
        ...
        maxValue = Math.round((MINUTES_OF_DAY / (double) timeStep + 1.0) * (1 - 0.01 * Scene.getInstance().getSolarHeatMapColorContrast()));
    ...
    private void computeOnLand(final ReadOnlyVector3 directionTowardSun) {
        ...
        for (int col = 0; col < cols; col++) {
            p.setX((col - cols / 2.0) * step + step / 2.0);
            for (int row = 0; row < rows; row++) {
                if (EnergyPanel.getInstance().isCancelled()) {
                    throw new CancellationException();
                }
                p.setY((row - rows / 2.0) * step + step / 2.0);
    ...
    private void computeOnMirror(final int minute, final ReadOnlyVector3 directionTowardSun, final Mirror mirror) {
        ...
        if (target != null) {
            towerCollisionMeshes = new ArrayList<>();
    ...
    private void computeOnFresnelReflector(final int minute, final ReadOnlyVector3 directionTowardSun, final FresnelReflector reflector) {
        ...
        if (target != null) {
            absorberCollisionMeshes = new ArrayList<>();
            for (final HousePart child : target.getChildren()) {
                ...
                if (dot > 0) {
                    ...
                    if (pickResults.getNumber() == 0) {
                        ...
                        if (absorber != null) {
                            ...
                            for (final Spatial spatial : collidables) {
                                if (spatial != mesh) {
                                    if (absorberCollisionMeshes == null || !absorberCollisionMeshes.contains(spatial)) {
    ...
    private MeshDataStore initMeshTextureData(final Mesh drawMesh, final Mesh collisionMesh, final ReadOnlyVector3 normal, final boolean updateTexture) {
        ...
        if (normal != null) {
            ...
            double z = Double.NaN;
            final List<ReadOnlyVector2> points = new ArrayList<>(vertexBuffer.limit() / 3);
        ...
        for (final HousePart part : Scene.getInstance().getParts()) {
            if (part instanceof Foundation) {
                ...
                for (int i = 0; i < n; i++) {
                    ...
                    if (groundHeatLoss > 0) {
                        final double thermostat = foundation.getThermostat().getTemperature(today.get(Calendar.MONTH),
                                today.get(Calendar.DAY_OF_WEEK) - Calendar.SUNDAY, today.get(Calendar.HOUR_OF_DAY));
    ...
    public void updateTextures() {
        ...
        maxValue = Math.round((MINUTES_OF_DAY / (double) Scene.getInstance().getTimeStep() + 1.0) * (1 - 0.01 * Scene.getInstance().getSolarHeatMapColorContrast()));
