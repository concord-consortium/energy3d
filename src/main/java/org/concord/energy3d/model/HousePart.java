...
public abstract class HousePart implements Serializable {
    ...
    static final double SNAP_DISTANCE = 0.5;
    static final double STRETCH_ROOF_STEP = 1;
    static final float printOutlineThickness = 2f;
    private static HousePart gridsHighlightedHousePart;
    private static boolean snapToObjects = true;
    private transient final int numOfDrawPoints;
    private transient final int numOfEditPoints;
    ...
    protected final ArrayList<HousePart> children = new ArrayList<>();
    ...
    static boolean isSnapToObjects() {
    ...
    public HousePart(final int numOfDrawPoints, final int numOfEditPoints, final double height, final boolean complete) {
        ...
        this.height = height;
        points = new ArrayList<>(numOfEditPoints);
        init();
        allocateNewPoint();
        if (complete) {
    ...
    private void addNewEditPointShape(final int i) {
        final Sphere pointShape = this instanceof SolarCollector ? new Sphere("Point", new Vector3(0, 0, 0.025), 8, 8, 0.05) : new Sphere("Point", Vector3.ZERO, 8, 8, 0.1);
    ...
    protected PickedHousePart pickContainer(final int x, final int y, final Class<?>[] typesOfHousePart) {
        ...
        if (!firstPointInserted && picked != null) {
            UserData userData = picked.getUserData();
            if (container == null || userData == null || container != userData.getHousePart()) {
                if (container != null) {
                    container.getChildren().remove(this);
                    if (this instanceof Roof) {
                        ((Wall) container).visitNeighbors((wall, prev, next) -> wall.setRoof(null));
                    }
                }
    ...
    Vector3 toRelativeVector(final ReadOnlyVector3 v) {
    ...
    Vector3 toAbsolute(final ReadOnlyVector3 p) {
        return toAbsolute(p, null);
    }

    Vector3 toAbsolute(final ReadOnlyVector3 p, final Vector3 result) {
        final HousePart container = getContainerRelative();
        if (container == null) {
    ...
    static ReadOnlyVector3 computeOrientedBoundingBox(final Mesh mesh) {
    ...
    private BMText fetchBMText(final String text, final int index) {
        final BMText label;
    ...
    SizeAnnotation fetchSizeAnnot(final int annotCounter) {
        return fetchSizeAnnot(annotCounter, sizeAnnotRoot);
    }

    SizeAnnotation fetchSizeAnnot(final int annotCounter, final Node sizeAnnotRoot) {
    ...
    AngleAnnotation fetchAngleAnnot(final int annotCounter) {
        return fetchAngleAnnot(annotCounter, angleAnnotRoot);
    }

    AngleAnnotation fetchAngleAnnot(final int annotCounter, final Node angleAnnotRoot) {
    ...
    public String toString() {
        StringBuilder s = new StringBuilder(this.getClass().getSimpleName() + "(" + id + ")");
        for (int i = 0; i < points.size(); i += 2) {
            s.append("  ").append(Util.toString2D(getAbsPoint(i)));
        }
        s.append("  editPoint=").append(editPointIndex);
        return s.toString();
    }

    void setLabelOffset(final double labelOffset) {
    ...
    ReadOnlyVector3 computeNormalAndKeepOnSurface() {
        if (container == null) {
            return null;
        }
        if (container instanceof Rack) {
            ...
            for (int i = 0; i < points.size(); i++) {
                ...
                if (points.size() > 1) {
                    ...
                    final Map<Integer, Integer> counts = new HashMap<>(points.size());
            ...
            if (nodes != null) {
                final Map<Vector3, ReadOnlyVector3> intersections = new HashMap<>();
                final PickResults pickResults = new PrimitivePickResults();
