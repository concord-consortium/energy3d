...
public class Floor extends HousePart {
    ...
    protected void drawMesh() {
        if (container != null) {
            if (wallUpperVectors == null) {
                wallUpperVectors = new ArrayList<>();
            } else {
    ...
    private ArrayList<PolygonPoint> exploreWallNeighbors(final Wall startWall) {
        final ArrayList<PolygonPoint> poly = new ArrayList<>();
        startWall.visitNeighbors((currentWall, prev, next) -> {
            int pointIndex = 0;
            if (next != null) {
