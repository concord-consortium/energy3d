...
class MyCylinder extends Mesh {
	 ...
	 */
	public MyCylinder(final String name, final int axisSamples, final int radialSamples, final double radius, final double height, final boolean closed, final boolean inverted) {
	...
	private void setGeometryData() {
		...
		for (int axisCount = 0, i = 0; axisCount < _axisSamples; axisCount++) {
			...
			for (int radialCount = 0; radialCount < _radialSamples; radialCount++) {
				...
				_meshData.getTextureCoords(0).getBuffer().put((float) (_inverted ? 1 - radialFraction : radialFraction)).put((float) axisFractionTexture);
				i++;
			}
