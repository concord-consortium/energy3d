...
public abstract class CameraControl {
    ...
    CameraControl(final ReadOnlyVector3 upAxis) {
        _upAxis.set(upAxis);
    }
    ...
    void removeTriggers(final LogicalLayer layer) {
    ...
    void setupMouseTriggers(final LogicalLayer layer, final boolean dragOnly) {
        // Mouse look
        ...
        layer.registerTrigger(new InputTrigger(new MouseWheelMovedCondition(),
                (source, inputStates, tpf) -> zoom(source, tpf, inputStates.getCurrent().getMouseState().getDwheel() * getCurrentExtent() / 20)));
    }
