...
public class SceneManager implements com.ardor3d.framework.Scene, Runnable, Updater {
			...
			public Object call() {
				try {
					...
					if (selectedPart instanceof Foundation) {
						...
						((Rack) selectedPart).ensureFullSolarPanels();
					}
					EnergyPanel.getInstance().update();
				} catch (final Throwable t) {
					t.printStackTrace();
	...
	public void setSolarHeatMap(final boolean solarHeatMap) {
		...
		EnergyPanel.getInstance().clearRadiationHeatMap();
		// FIXME: (Xie) For some reason, rack's texture doesn't change correctly until it is redrawn. Rack has to change between a repeated texture (solar panel) and a single texture (solar radiation)
		SceneManager.getTaskManager().update(new Callable<Object>() {
			@Override
			public Object call() throws Exception {
