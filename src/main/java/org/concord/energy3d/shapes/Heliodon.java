...
public class Heliodon {
	...
	public Heliodon(final Node scene, final DirectionalLight light, final BasicPassManager passManager, final LogicalLayer logicalLayer, final Date timeAndDate) {
		...
		setDate(timeAndDate);
		PropertiesPanel.getInstance().getDateSpinner().setValue(timeAndDate);
		setTime(timeAndDate);
		...
		if (isNightTime()) {
			...
			PropertiesPanel.getInstance().getTimeSpinner().setValue(calendar.getTime());
		} else
			PropertiesPanel.getInstance().getTimeSpinner().setValue(timeAndDate);
	...
	public void setDeclinationAngle(final double declinationAngle, final boolean redrawHeliodon, final boolean updateGUI) {
		...
		if (updateGUI) {
			...
			PropertiesPanel.getInstance().getDateSpinner().setValue(calendar.getTime());
		}
